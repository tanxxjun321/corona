...
目标：隐藏图标可被临时移动到可见区、点击打开，再自动回到原分区。
...
- auto-rehide 策略：时间、前台应用变化、用户输入暂停、菜单弹窗关闭。
- click executor：左键、右键、备用点击。
...
- Screen Recording 增强模式：真实像素缩略图和菜单栏背景取色。
- 完整自动化测试和手工验收清单。
